...
def regular_solution(find_for_auto_sign):
    ...
    for i in find_for_auto_sign:
        result += re.findall(r'^[ABKMTXOАВЕКМНОРСТУХ]\d{3}[ABKMTXOАВЕКМНОРСТУХ]{2}\d{2,3}', i)
    return result
...
def main():
    ...
    # решение 1
    print("регулярное выражение: ", regular_solution(find_for_auto_sign), end = "\n")
    # решение 2
    print("проброс через булево условие: ", get_of_valid_sign(find_for_auto_sign))
